...
export default class ProfilesSorter implements MetadataSorter<Profile> {
	sortMetadata(profile: Profile | null): Profile {
		...
		if (profile.applicationVisibilities != null) {
			profile.applicationVisibilities = this.sortApplicationVisibilities(
				profile.applicationVisibilities
			);
		}
		if (profile.categoryGroupVisibilities != null) {
			profile.categoryGroupVisibilities = this.sortCategoryGroupVisibilities(
		...
		if (profile.customMetadataTypeAccesses != null) {
			profile.customMetadataTypeAccesses = this.sortCustomMetadataTypeAccesses(
				profile.customMetadataTypeAccesses
			);
		}
		if (profile.customPermissions != null) {
			profile.customPermissions = this.sortCustomPermissions(
				profile.customPermissions
			);
		}
		if (profile.externalDataSourceAccesses != null) {
			profile.externalDataSourceAccesses = this.sortExternalDataSourceAccesses(
				profile.externalDataSourceAccesses
			);
		}
		if (profile.fieldLevelSecurities != null) {
			profile.fieldLevelSecurities = this.sortFieldLevelSecurities(
				profile.fieldLevelSecurities
			);
		}
		if (profile.fieldPermissions != null) {
			profile.fieldPermissions = this.sortFieldPermissions(
		...
		if (profile.layoutAssignments != null) {
			profile.layoutAssignments = this.sortLayoutAssignments(
		...
		if (profile.objectPermissions != null) {
			profile.objectPermissions = this.sortObjectPermissions(
		...
		if (profile.profileActionOverrides != null) {
			profile.profileActionOverrides = this.sortProfileActionOverrides(
				profile.profileActionOverrides
			);
		}
		if (profile.recordTypeVisibilities != null) {
			profile.recordTypeVisibilities = this.sortRecordTypeVisibilities(
				profile.recordTypeVisibilities
			);
		}
		if (profile.tabVisibilities != null) {
			profile.tabVisibilities = this.sortTabVisibilities(
				profile.tabVisibilities
			);
		}
		if (profile.userPermissions != null) {
			profile.userPermissions = this.sortUserPermissions(
	...
	private sortApplicationVisibilities(
	...
	private sortCategoryGroupVisibilities(
		categoryGroupVisibilities: ProfileCategoryGroupVisibility[]
	): ProfileCategoryGroupVisibility[] {
		return categoryGroupVisibilities.map((category) =>
			sortObjectPropertiesAlphabetically(category)
		);
	}

	private sortClassAccesses(
		classesAccesses: ProfileApexClassAccess[]
	...
	private sortCustomMetadataTypeAccesses(
		...
		return customMetadataTypeAccesses
			.map((customMetadataTypeAccess) =>
				sortObjectPropertiesAlphabetically(customMetadataTypeAccess)
			)
			.sort((a, b) => compareByField(a, b, "name"));
	}

	private sortCustomPermissions(
		customPermissions: ProfileCustomPermissions[]
	): ProfileCustomPermissions[] {
		return customPermissions
			.map((customPermission) =>
				sortObjectPropertiesAlphabetically(customPermission)
			)
			.sort((a, b) => compareByField<ProfileCustomPermissions>(a, b, "name"));
	}

	private sortExternalDataSourceAccesses(
	...
	private sortFieldLevelSecurities(
		fieldLevelSecurities: ProfileFieldLevelSecurity[]
	): ProfileFieldLevelSecurity[] {
		return fieldLevelSecurities
			.map((fieldLevelSecurity) =>
				sortObjectPropertiesAlphabetically(fieldLevelSecurity)
			)
			.sort((a, b) => compareByField(a, b, "field"));
	}

	private sortFieldPermissions(
		fieldPermissions: ProfileFieldLevelSecurity[]
	): ProfileFieldLevelSecurity[] {
		return fieldPermissions
			.map((fieldPermission) =>
				sortObjectPropertiesAlphabetically(fieldPermission)
			)
			.sort((a, b) => compareByField(a, b, "field"));
	}

	private sortFlowAccesses(
	...
	private sortLoginFlows = (loginFlows: LoginFlow[]) => loginFlows;

	private sortLoginHours = (loginHours: ProfileLoginHours[]) => loginHours;

	private sortLoginIpRanges = (loginIpRanges: ProfileLoginIpRange[]) =>
		loginIpRanges;

	private sortObjectPermissions(
		objectPermissions: ProfileObjectPermissions[]
	): ProfileObjectPermissions[] {
		return objectPermissions
			.map((objectPermission) =>
				sortObjectPropertiesAlphabetically(objectPermission)
			)
			.sort((a, b) => compareByField(a, b, "object"));
	}

	private sortPageAccesses(
	...
	private sortProfileActionOverrides(
		profileActionOverrides: ProfileActionOverride[]
	): ProfileActionOverride[] {
		return profileActionOverrides.map((profileActionOverride) =>
			sortObjectPropertiesAlphabetically(profileActionOverride)
		);
	}

	private sortRecordTypeVisibilities(
		recordTypeVisibilities: ProfileRecordTypeVisibility[]
	): ProfileRecordTypeVisibility[] {
		return recordTypeVisibilities
			.map((recordTypeVisibility) =>
				sortObjectPropertiesAlphabetically(recordTypeVisibility)
			)
			.sort((a, b) => compareByField(a, b, "recordType"));
	}

	private sortTabVisibilities(
	...
	private sortUserPermissions(
		userPermissions: ProfileUserPermission[]
	): ProfileUserPermission[] {
		return userPermissions
			.map((userPermission) =>
				sortObjectPropertiesAlphabetically(userPermission)
			)
			.sort((a, b) => compareByField(a, b, "name"));
	}
}
